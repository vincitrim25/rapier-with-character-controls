...
import { usePhysics, useRenderSize, useRenderer } from "../init";
import { PhysicsObject, addPhysics } from "../physics/physics";
import { UpDownCirc, clamp, easeOutExpo, lerp } from "./utils/math";
...
class InputManager {
    ...
    init() {
        ...
        this.target.addEventListener('wheel', (e) => this.onMouseWheel(e), false);
            ...
            if (this.pointerLocked) {
                if (e.deltaY < 0) {
                    this.currentMouse.mouseWheelDelta = Math.max(
                        this.currentMouse.mouseWheelDelta - SCROLL_LEVEL_STEP, MIN_ZOOM_LEVEL
                    )
                }
                else if (e.deltaY > 0) {
                    this.currentMouse.mouseWheelDelta = Math.min(
                        this.currentMouse.mouseWheelDelta + SCROLL_LEVEL_STEP, MAX_ZOOM_LEVEL
                    )
                }
            }
        }
